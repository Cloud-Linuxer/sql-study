...
export default function SchemaViewerAPI({ schema }) {
  ...
  useEffect(() => {
    ...
    // Fetch sample data for first 10 columns
    async function fetchSamples() {
      const samples = {};

      for (const col of schema.columns.slice(1, 11)) { // Skip id column
        try {
          const response = await fetch(`/api/sample/${encodeURIComponent(col.name)}`);
          if (response.ok) {
            samples[col.name] = await response.json();
          }
        } catch (err) {
          console.warn(`Failed to fetch sample for ${col.name}`);
        }
      }
          ...
          <div className="mb-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-gray-500 dark:text-gray-400">
                ({schema.columns.length} 컬럼, {schema.rowCount?.toLocaleString('ko-KR')} 행)
              </span>
            </div>
          </div>

          <div className="space-y-2">
            {schema.columns.slice(1).map((col, idx) => (
              <div
                key={idx}
